...
describe("beforeSuggest callback", function() {
...
describe("suggest callback", function() {
  it("should be called with the correct arguments", function() {
    var stickySuggest   = jasmine.createSpy("stickySuggest");
    var stickySuccess   = jasmine.createSpy("stickySuccess");
    var stickyError     = jasmine.createSpy("stickyError");
      ...
      experiments: [
        {
          ...
          sticky: true,
          callbacks: {
            suggest: stickySuggest
          }
        },
        {
          uuid: 'non-stick-uuid2',
          class: 'non-sticky-class',
          sticky: false,
          callbacks: {
            suggest: nonStickSuggest
          }
        }
      ]
    });
    ...
    runs(function() {
      myna.clearSuggestions();
      myna.suggest('sticky-uuid2', stickySuccess, stickyError);
      myna.suggest('non-stick-uuid2', nonStickSuccess, nonStickError);
    });
    ...
    runs(function() {
      ...
      expect(stickySuggest.argsForCall[0]).toEqual([
        'variant1',
        { uuid: 'sticky-uuid2', choice: 'variant1', token: 'token1', skipped: false, rewarded: false },
        true
      ]);
      expect(stickySuggest.argsForCall[1]).toEqual([
        'variant1',
      ...
      // expect(nonStickSuggest.argsForCall[0]).toEqual([
      //   'variant1',
      //   { uuid: 'non-stick-uuid2', choice: 'variant1', token: 'token1', skipped: false, rewarded: false },
      //   true
      // ]);
      // expect(nonStickSuggest.argsForCall[1]).toEqual([
      //   'variant2',
      //   { uuid: 'non-stick-uuid2', choice: 'variant2', token: 'token2', skipped: false, rewarded: false },
      //   true
      // ]);
