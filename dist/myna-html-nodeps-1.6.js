/*!
 * Myna for HTML v1.6 (no dependencies)
  ...
  return Myna = (function() {
    ...
    Myna.defaults = {
      ...
      callbacks: {
        ...
        suggest: (function(variant, stored, fromCookie) {}),
        target: null
      }
    };
    ...
    function Myna(options) {
      var exptDefaults, expts, _ref, _ref1, _ref2, _ref3, _ref4,
        _this = this;
      if (options == null) {
        options = {};
      }
      ...
      exptDefaults = {
        ...
        callbacks: {
          ...
          suggest: (_ref2 = this.options.callbacks) != null ? _ref2.suggest : void 0,
          target: ((_ref3 = this.options.callbacks) != null ? _ref3.target : void 0) ? (_ref4 = this.options.callbacks) != null ? _ref4.target : void 0 : this.options.skipChance ? function() {
      ...
      $.each(this.options.experiments, function(index, options) {
        var cssClass, googleDefaults, sticky, uuid, _ref5;
        uuid = options.uuid;
        cssClass = options['class'];
        sticky = options.sticky;
        if (options.skipChance) {
          if (!((_ref5 = options.callbacks) != null ? _ref5.target : void 0)) {
    ...
    Myna.prototype.ajax = function(url, success, error) {
      var errorTimer, exn, myna, resolved, timeout, wrappedError, wrappedSuccess, xhr;
    ...
    Myna.prototype.trackGoogleSuggestEvent = function(uuid, choice) {
      ...
      if (enabled) {
        return typeof _gaq !== "undefined" && _gaq !== null ? _gaq.push(["_trackEvent", "myna", eventName, choice, null, false]) : void 0;
      }
    };

    Myna.prototype.trackGoogleRewardEvent = function(uuid, choice, amount) {
      var enabled, eventName, naturalAmount;
      enabled = this.exptGoogleOption(uuid, "enabled", (function() {
        return true;
      }));
      ...
      if (enabled) {
        return typeof _gaq !== "undefined" && _gaq !== null ? _gaq.push(["_trackEvent", "myna", eventName, choice, naturalAmount, true]) : void 0;
      }
    };
    ...
    Myna.prototype.suggest = function(uuid, success, error) {
      var afterSuggest, beforeSuggest, sticky, stored, successWrapper;
      if (success == null) {
        success = (function() {});
      }
      if (error == null) {
      ...
      afterSuggest = this.exptCallback(uuid, "suggest", (function() {
        return function() {};
      }));
      successWrapper = function(stored) {
        beforeSuggest(stored, true);
        success(stored);
        return afterSuggest(stored.choice, stored, true);
      };
      if (stored) {
        beforeSuggest(stored, false);
        success(stored);
        return afterSuggest(stored.choice, stored, false);
